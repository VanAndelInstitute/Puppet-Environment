...
# separators for version and package processing
$version_separator = "~"
$package_separator = "|"
...
def setup()
  ...
  # determine where to store facts based on the OS
	...
	filepath = windows_filepath if $os.include? "windows"

  config_file = "#{filepath}#{$fqdn}_configuration.yaml"
  ...
  # otherwise, a configuration file exists, check for inconsistency
    ...
    found_packages =  $current 
  	...
  	prev_drift = Facter.value(:drift) 
		...
		if (formatWhitespace(found_packages) == formatWhitespace(init_config))
    ...
    # otherwise drift is detected
      ...
      found_packages.gsub!($package_separator, "")
      init_config.gsub!($package_separator, "\n")
      init_config.gsub!("\"", "")
      ...
      init_config.each_line do |line|
        ...
        line.strip!
				...
				unless found_packages[line]				
					drift = true
					name = line.split($version_separator)
					...
					if(name.length == 1)
						...
						name = line.split($version_separator)[0]
						ver = line.split($version_separator)[1]
					end
      ...
      found_packages.each_line do |pack|
				name = pack.split($version_separator)[0]
        ...
        unless init_config.include? pack
          ...
          Puppet.notice(notice)
			...
			if drift
				...
				prev_drift = Facter.value(:drift)
				unless (formatWhitespace(prev_drift) == formatWhitespace($curr_drift))
...
def currentConfig()
    ...
    packages.each do |pack|
      ...
      name = pack.split("'")[1]
			...
			unless name.include? version || name.nil? || version.nil?
				package = name + $version_separator + version
			else
				...
				if !name.nil? and !name.empty?
					...
					if length > 1 
						...
						package = name + $version_separator + ver
					end
				end
			end
      ...
      unless package.nil? or package.include? "ensure" or package.empty? or (package.length < 2)
		...
		str += "\nip#{$version_separator}#{ip}#{$package_separator}" unless ip.nil?
		str += "\nmac#{$version_separator}#{mac}#{$package_separator}" unless mac.nil?
